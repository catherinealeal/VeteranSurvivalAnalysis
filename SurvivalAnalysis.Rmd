...
```{r}
covariates <- c("trt", "celltype", "karno", "diagtime", "age", "prior")
for (cov in covariates) {
  model <- survreg(as.formula(paste("surv_obj ~", cov)), 
                   data = veteran, 
                   dist = "weibull")
  print(cov)
  print(summary(model)$table)}
```
...
```{r}
para_model = survreg(surv_obj ~ celltype + karno, data = veteran, dist = "weibull")
summary(para_model)
```
...
```{r}
km_cell <- survfit(surv_obj ~ celltype, data = veteran)
plot(km_cell, col = 1:4, lty = 1:4,
     xlab = "Days", ylab = "Survival Probability", 
     main = "Survival by Cell Type")
legend("topright", legend = levels(veteran$celltype), col = 1:4, lty = 1:4)
```
...
```{r}
survdiff(surv_obj ~ celltype, data = veteran)
```
...
```{r}
exp_fit <- survreg(Surv(time, status) ~ 1, data = veteran, dist = "exponential")
time_grid <- seq(0, max(veteran$time), length.out = 200)
rate <- 1 / exp(coef(para_model))  
S <- exp(-rate * time_grid)

plot(km_fit, xlab = "Days", ylab = "Survival Probability",
     main = "KM vs Exponential Fit", lwd = 2)
lines(time_grid, S, col = "red", lwd = 2)
legend("topright", legend = c("KM", "Exponential"),
